...
export default function UbicacioEnServei(){
  ...
  useEffect(()=>{
    ...
    if(!id)return;
    const servei=serveis.find(s=>s.id===id);
    llistarVoluntaris(servei?.agrupacioId).then(v=>{if(!cancelat)setVoluntaris(v.filter(x=>x.actiu));}).catch(()=>{if(!cancelat)setError('No s’ha pogut carregar el voluntariat');});
  ...
  return <div>
    ...
    <label>Servei<select aria-label="Servei del mapa" value={id} onChange={e=>setId(e.target.value)} style={{width:'100%'}}><option value="">Selecciona un servei…</option>{serveis.map(s=><option key={s.id} value={s.id}>{s.titol} · {new Date(s.dataInici).toLocaleDateString('ca-ES')}</option>)}</select></label>
    ...
    {editar && <form className="card" style={{marginTop:12}} onSubmit={e=>{e.preventDefault();desar();}}>
      <label>Voluntari<select required aria-label="Voluntari del punt" value={form.voluntariId} onChange={e=>setForm({...buit,voluntariId:e.target.value})}><option value="">Selecciona…</option>{voluntaris.map(v=><option key={v.id} value={v.id}>{v.nom} {v.cognoms}</option>)}</select></label>
